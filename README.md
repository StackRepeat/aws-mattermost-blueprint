...
Automatic **Open workload** linking requires **AWS runtime v2.3.58 or later**.
[Release v2.3.58](https://github.com/StackRepeat/aws-platform/tree/v2.3.58)
includes [PR #72](https://github.com/StackRepeat/aws-platform/pull/72), which reads
Terraform's non-sensitive `endpoint` output after a successful post hook.
Upgrade the platform through the console so both the executor buildspec and
runtime Lambda image are updated. Runtime `v2.3.57` predates the change. An older
...
With Terraform 1.15.8 or a later 1.15 patch, AWS CLI credentials for a suitable test account, and an
approved region:
